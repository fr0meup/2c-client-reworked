...
Latest Android release: **v0.2.8**. See the [release notes](https://github.com/fr0meup/2c-client-reworked/releases/tag/v0.2.8) for details.
...
- Searchable stock tickers and tappable `$SYMBOL` references in posts and comments, with company details, price charts, and ticker-specific post and reply feeds.
...
Discovery uses half of v0.1.8's concurrency in each main scan phase, targeting roughly twice that version's duration under comparable conditions without a mandatory delay between requests. If the service rate-limits the scan, it pauses, respects any server-provided retry delay, and temporarily resumes more slowly before recovering after successful requests. Progress in the followers sheet or background toast shows when it is paused. Network conditions and service limits still affect total time; rate limiting cannot be guaranteed away. Normal browsing and interactive advanced search do not use this scan limiter.
...
In **Edit profile**, optionally follow yourself and choose a nickname. Self-follow is off by default unless you already follow yourself. You can change the nickname or turn self-follow off there, then select **Save changes**. Nickname changes take time to appear throughout the app; refreshing or reopening pages will not speed this up.
